...
export function isFormError(error: unknown): error is FormErrorCause {
  return (
    typeof error === 'object'
    && error !== null
    && !Array.isArray(error)
    && Object.values(error).every(v => Array.isArray(v) && v.every(v => typeof v === 'string'))
  )
}
...
export function isHttpResponse(response: unknown): response is HttpResponse {
  return response instanceof Response && 'data' in response && 'error' in response
}
